...
```json
{"line":1,"mode":"demo","value":0,"priority":"low","actionableProbability":0.01,"route":"retain","reason":"model","cached":false,"fingerprint":"GET /health returned 200 in <*>"}
```
...
| `rule` | Name of the matching rule when `reason` is `rule` |
...
| `cache` | 1,000 entries, 5 minutes | `{ maxEntries, ttlMs }` or `false`; keyed by fingerprint when identifiers were masked, otherwise by the redacted model input; failures are never cached |
| `fingerprint` | `true` | Replace identifiers with `<*>` and share cached decisions across matching templates. `false` uses exact redacted input only |
...
| `jev.fingerprint` | Identifier template, when fingerprinting is enabled |
| `jev.fingerprint_hits` | Process-local reuse count for that fingerprint, including this record |
...
| Two similar logs both called Jev | Fingerprints mask identifiers, not status codes or small numbers. `fingerprint: false` forces exact-input caching. A template with too little remaining text is not shared. |
